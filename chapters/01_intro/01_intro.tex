...
\chapter{Introduction}
	...
	\begin{algorithm}
		...
		\[ P_x = \left\langle \left| \frac{1}{N} \sum_{i=1}^N \cos(\theta_i) \right| \right\rangle ;
		...
		\begin{figure}[htp]
			\centering
			\subfloat[][Polarization and its susceptibility]{\includegraphics[width=0.45\textwidth]{martin_phasetransition.png}}
		    \subfloat[][Pair correlation function and static structure factor]{\includegraphics[width=0.45\textwidth]{martin_gofr.png}}
		...
		In the aforementioned paper, it is shown how, when ABPs systems with a simple repulsive only Weeks-Chandler-Anderson potential phase-separate in a cluster, their velocity tend to form aligned domains, regardless the self propulsion orientation. For this reason, the global polarization $\left| \sum_{k=1}^{N} \exp{i \psi_k(t)} \right| / N$, where $\psi_k(t)$ is the orientation angle of particle $k$ at time $t$, is not a good order parameter, even when the computation is restricted to clusters, thus the authors introduce the spatial correlation function of the velocity orientation $Q_i(r) = 1-2\sum_{j} \frac{d{ij}}{\mathcal{N}_k \pi } $, being $d_{ij} = \mathrm{min}\left[|\psi_i - \psi_j|, 2pi - |\psi_i - \psi_j| \right]$ the angular distance between two particles and $\mathcal{N}_k$ is the number of particles in a circular shell around the i-th particle, taken with a mean radius. 
