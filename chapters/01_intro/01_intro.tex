...
\begin{document}
	...
	\begin{comment}
		...
		Given that a typical BP will have a characteristic body-length in the order of $ \mathrm{\sim\mu m}$ and a velocity of $\sim\mu \mathrm{m s}^{-1}$ the system can be studied in low-Reynolds number regime being 
		...
		With this hypothesis, rotational diffusion has now become relevant since the direction of self-propulsion varies randomly with a characteristic time scale which corresponds to the inverse of the rotational diffusion coefficient $\tau_r = D_r^{-1}$. Adding self-propulsion into the equations of motion, we get:
		\begin{equation}
			\dot{x} = v \cos{\theta} + \sqrt{2D_t}dW_x , \quad \dot{y} = v \sin{\theta} + \sqrt{2D_t}dW_y, \quad \dot{\theta} = \sqrt{2D_r}dW_{\theta}
		\end{equation}
		
		Starting from this fact, it is possible to derive the fundamental features to take into account in the simulation:
		\begin{itemize}
			\item[(i)] directionality over a characteristic time interval
			\item[(ii)] orientational noise
			\item[(iii)] absence of inertia
		\end{itemize}
	\end{comment}
