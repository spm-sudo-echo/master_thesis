 ...
 \]
  ...
  \label{fig:martin_flocking3}
	...
	\subsection[ABP simulations]{Physics and Numerical Simulations of ABPs}
		...
		\end{equation}
		in which we assumed 0 mean and no correlation at different times, which reads $\dd{W} = \xi(t)\dd{t}$ with $ \langle \xi(t) \xi(t') \rangle = \delta(t-t')$.
	    ...
	    \end{align}
	    with $z_1 \sim \mathcal{N}(0,1)$, and we notice that the last term is $\order{h^{3/2}}$, leaving us with the first order.
	    
	    This makes what we saw for the finite difference equations correct.
	    
		\subsection{Algorithms}
		The topic of algorithms to integrate SDEs is pretty broad and quite a lot of literature has been produced on the subject. Here, we only give the details of what has been used in present work.
